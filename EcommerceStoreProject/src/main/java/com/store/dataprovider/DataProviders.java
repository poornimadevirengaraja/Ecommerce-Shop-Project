...
import com.store.utilities.NewExcelLibrary;
/**
 * 
 * End to End Selenium Framework | E-Commerce Project | 
 * Complete Selenium Framework from Scratch| Data Providers class
 * Author - PoornimaDevi 
 * 
 * **/
...
//Class --> AddToCartPageTest, EndToEndTest,  Test Case--> addToCartTest, endToEndTest	
	...
	public Object[][] getProduct() {
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				...
				//System.out.println(data[i][j] + " i="+i+" j="+j);
			}
		}
		return data;
	}
	...
	public Object[][] accountCreation() {
		...
		for (int i = 0; i < actRows; i++) {
			...
			data[i][0]=hashMap;
			//System.out.println("i = "+ i + data[i][0]);
		}
		return data;
	}
	...
	public Object[][] EndtoEnd() {
		// Totals rows count
		int rows = obj.getRowCount("EndtoEnd");
		// Total Columns
		int column = obj.getColumnCount("EndtoEnd");
		int actRows = rows - 1;
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				data[i][j] = obj.getCellData("EndtoEnd", j, i + 2);
				//System.out.println(data[i][j] + " i="+i+" j="+j);
			}
		}
		return data;
